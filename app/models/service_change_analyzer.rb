  ...
  def self.service_change_summary(route_id, actual_route_directions, scheduled_routings, current_routings, recent_routings, evergreen_routings, transfers)
    ...
    condensed_changes[1].select! do |c1|
      if other_direction = condensed_changes[0].find { |c2|
            c1.class == c2.class &&
            (c1.first_station == c2.last_station || transfers[c1.first_station]&.any?{ |t| t.from_stop_internal_id == c2.last_station }) &&
            (c1.last_station == c2.first_station || transfers[c1.last_station]&.any?{ |t| t.from_stop_internal_id == c2.first_station }) &&
            c1.class != SplitRoutingServiceChange
          }
