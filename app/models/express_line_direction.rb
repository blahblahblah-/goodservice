class ExpressLineDirection < LineDirection
  ...
  def scheduled_runtimes
    ...
    local_stop = recent_stop_times_by_stops(local_line_direction.last_stop)
    ...
    if line.name == "Broadway (Brooklyn)" # line has no branches, so will need to take address that
      ...
      first.concat(StopTime.recent(time_range: 1.hour).joins(:trip).where(stop_internal_id: "#{first_stop[0..2]}#{new_direction}", trips: {route_internal_id: 'M'}).to_a)
      local_stop.concat(StopTime.recent(time_range: 1.hour).joins(:trip).where(stop_internal_id: "#{local_line_direction.last_stop[0..2]}#{new_direction}", trips: {route_internal_id: 'M'}).to_a)
      last.concat(StopTime.recent.joins(:trip).where(stop_internal_id: "#{last_stop[0..2]}#{new_direction}", trips: {route_internal_id: 'M'}).to_a)
    end

    last.reject! { |last_st|
      local_stop.any? { |p_st| p_st.trip_internal_id == last_st.trip_internal_id } || first.none? { |first_st| first_st.trip_internal_id == last_st.trip_internal_id }
    }
  ...
  def actual_runtimes
    ...
    local_stop = Rails.cache.read("stoptime-#{local_line_direction.last_stop}") || {}
    ...
    last.each { |ls_stop_id, ls_trips|
      ls_trips.reject! { |trip_id, time|
        first.all? { |_, v| v[trip_id].nil? } || local_stop[trip_id].present?
      }
    }
  ...
  def scheduled_throughput
    ...
    if line.name == "Broadway (Brooklyn)" # line has no branches, so will need to take address that
      ...
      local_stop.concat(StopTime.recent(time_range: 2.hours).joins(:trip).where(stop_internal_id: "#{local_line_direction.last_stop[0..2]}#{new_direction}", trips: {route_internal_id: 'M'}).to_a)
      last.concat(StopTime.recent(time_range: 1.hour).joins(:trip).where(stop_internal_id: "#{last_stop[0..2]}#{new_direction}", trips: {route_internal_id: 'M'}).to_a)
    end

    last.reject! { |last_st|
      local_stop.any? { |p_st| p_st.trip_internal_id == last_st.trip_internal_id }
    }
  ...
  def actual_throughput
    ...
    return @actual_throughput = 0 unless last.any?(&:present?)
