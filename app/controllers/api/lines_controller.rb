class Api::LinesController < ApplicationController
  ...
  caches_action :archive, expires_in: 2.hour
  ...
  def archive
    ...
    summary = LineStatusSummary.find_by(line_id: params[:line_id], date: date)

    return render json: summary.summary if summary
    ...
    render json: {
      north: {
        local: Api::LinesController.travel_time_stats_archive(lds.find {|ld| ld.type != "ExpressLineDirection" && ld.direction == 1 }, from_time, to_time),
        express: Api::LinesController.travel_time_stats_archive(lds.find {|ld| ld.type == "ExpressLineDirection" && ld.direction == 1 }, from_time, to_time)
      },
      south: {
        local: Api::LinesController.travel_time_stats_archive(lds.find {|ld| ld.type != "ExpressLineDirection" && ld.direction == 3 }, from_time, to_time),
        express: Api::LinesController.travel_time_stats_archive(lds.find {|ld| ld.type == "ExpressLineDirection" && ld.direction == 3 }, from_time, to_time)
      }
    }
  end

  def self.travel_time_stats_archive(line_direction, from_time, to_time)
