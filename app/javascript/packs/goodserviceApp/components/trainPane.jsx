...
class TrainPane extends React.Component {
  ...
  panes() {
    const { width, trains, trainStats, onFavTrainChange, favTrains, routing, stops } = this.props;
    return [
      { menuItem: 'Current Status', render: () =>
        <Tab.Pane attached={false} basic={true} key='stats' style={{padding: '1em 0'}}>
          <TrainStatusPane trains={trains} trainStats={trainStats} routing={routing} stops={stops}
